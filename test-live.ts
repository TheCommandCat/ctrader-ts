...
import { DEMO_HOST, API_PORT } from "./src/config.js";
...
async function main() {
  ...
  try {
    ...
    for (const a of accounts) {
      console.log(
        `    Account ${a.ctidTraderAccountId} (${a.isLive ? "LIVE" : "DEMO"})`,
      );
    }
  } catch (err) {
  ...
  try {
    ...
    ok(
      `${positions.length} open position(s), ${orders.length} pending order(s)`,
    );
  } catch (err) {
    fail("Reconcile", err);
  }
  ...
  if (eurusdId) {
    ...
    try {
      ...
      const timeout = new Promise<void>((_, reject) =>
        setTimeout(
          () => reject(new Error("Timed out waiting for 3 ticks (15s)")),
          15_000,
        ),
      );
