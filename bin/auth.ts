...
import { saveConfig, loadStoredConfig, getConfigPath, DEMO_ENDPOINT, type Environment } from "../src/config.js";
...
async function exchangeCodeForToken(
	...
	if (body["errorCode"]) throw new Error(`${body["errorCode"]}: ${body["description"]}`);
...
async function run(): Promise<void> {
	...
	if (stored.accessToken && stored.refreshToken) {
		...
		if (p.isCancel(refresh)) { p.cancel("Cancelled."); process.exit(0); }

		if (!refresh) {
			accessToken = stored.accessToken;
			refreshToken = stored.refreshToken;
		} else {
			({ accessToken, refreshToken } = await doOAuthFlow(clientId as string, clientSecret as string));
		}
	} else {
		({ accessToken, refreshToken } = await doOAuthFlow(clientId as string, clientSecret as string));
	}
	...
	if (p.isCancel(accountChoice)) { p.cancel("Cancelled."); process.exit(0); }
...
async function doOAuthFlow(clientId: string, clientSecret: string): Promise<{ accessToken: string; refreshToken: string }> {
	...
	if (p.isCancel(redirected)) { p.cancel("Cancelled."); process.exit(0); }
