...
const ERROR_HINTS: Record<string, string> = {
	...
	ACCOUNT_NOT_AUTHORIZED:
		"This account isn't authorized. Run `ctrader-ts auth` and select the correct account.",
	CH_CLIENT_AUTH_FAILURE:
		"Client authentication failed. Check your clientId and clientSecret in config.",
	CH_CLIENT_NOT_AUTHENTICATED:
		"You must authenticate before making requests. Call connect() which handles this automatically.",
	CH_ACCESS_TOKEN_INVALID: "Invalid access token. Run `ctrader-ts auth` to get a fresh token.",
	OAUTH_TOKEN_INVALIDATED:
		"Token was invalidated (possibly revoked or used from another location). Run `ctrader-ts auth`.",

	// Rate limiting
	REQUEST_FREQUENCY_EXCEEDED:
		"Too many requests. Wait and retry — consider adding delays between rapid API calls.",
	BLOCKED_PAYLOAD_TYPE:
		"This request type is temporarily blocked due to rate limiting. Wait before retrying.",

	// Maintenance
	SERVER_IS_UNDER_MAINTENANCE:
		"Server is under maintenance. Check maintenanceEndTimestamp for expected end time.",

	// Trading errors
	NOT_ENOUGH_MONEY:
		"Insufficient free margin. Reduce position size or close existing positions to free up margin.",
	SYMBOL_NOT_FOUND:
		"Symbol not found. Check available symbols with getSymbols() or `ctrader-ts symbols`.",
	INVALID_VOLUME:
		"Invalid volume. Ensure lots >= minVolume and is a multiple of stepVolume. Use getSymbolInfo() to check limits.",
	MARKET_CLOSED: "Market is closed for this symbol. Check trading hours with getSymbolInfo().",
	TRADING_BAD_VOLUME:
		"Volume out of range. Use getSymbolInfo() to check minVolume, maxVolume, and stepVolume.",
	TRADING_BAD_STOPS:
		"Invalid SL/TP distance. Some brokers require minimum distances — use getSymbolInfo() to check slDistance/tpDistance.",
	...
	NO_MORE_ACCOUNTS_AVAILABLE:
		"No more accounts available. Check your cTrader ID has linked broker accounts.",
	CONNECTIONS_LIMIT_EXCEEDED:
		"Too many connections from this app. Close other sessions or wait for them to expire.",
...
export class RequestTimeoutError extends Error {
	...
	constructor(payloadType: number, clientMsgId: string, timeoutMs: number) {
		super(
			`Request timed out after ${timeoutMs}ms (payloadType: ${payloadType}, id: ${clientMsgId}). Check your network connection or try increasing requestTimeoutMs.`,
		);
