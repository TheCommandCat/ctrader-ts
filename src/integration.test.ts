...
import { CTrader, lotsToUnits } from "./client.js";
import { TradeSide, TrendbarPeriod, QuoteType } from "./enums.js";
import type { Position, FullSymbol, LightSymbol, ExecutionEvent } from "./types.js";
...
describe("integration", () => {
  ...
  test("getTrader returns valid trader info", async () => {
  ...
  test("getState returns complete account snapshot", async () => {
  ...
  test("getPositions returns positions and orders arrays", async () => {
  ...
  test("getSymbols returns non-empty list with EURUSD", async () => {
  ...
  test("getSymbolDetails returns full symbol info", async () => {
  ...
  test("getSymbolDetails by numeric ID", async () => {
  ...
  test("getTrendbars returns historical candles", async () => {
    const to = Date.now();
  ...
  test("getTickData returns historical ticks", async () => {
    const to = Date.now();
  ...
  test("cleanup: close existing EURUSD positions", async () => {
    try {
  ...
  test("buy opens a BUY position", async () => {
  ...
  test("position appears in getState", async () => {
    const state = await ct.getState();
  ...
  test("modify adds SL/TP to position", async () => {
    ...
    if (pos!.stopLoss !== undefined) {
      ...
      expect(pos!.stopLoss!).toBeLessThan(pos!.price!);
      expect(pos!.takeProfit!).toBeGreaterThan(pos!.price!);
    }
  }, 30_000);

  test("resize increases position volume", async () => {
  ...
  test("resize decreases position volume", async () => {
  ...
  test("resize no-op when same volume", async () => {
    const result = await ct.resize(openedPosition.positionId, LOT_SIZE);
    expect(result).toBeUndefined();
  }, T);

  test("close fully closes the position", async () => {
  ...
  test("sell opens a short position", async () => {
  ...
  test("partial close reduces volume", async () => {
  ...
  test("close remaining short position", async () => {
  ...
  test("buyLimit places a pending order", async () => {
  ...
  test("pending order appears in getPositions", async () => {
    const { orders } = await ct.getPositions();
    const found = orders.find(
      (o) => o.tradeData.symbolId === eurusdId && o.limitPrice !== undefined && o.limitPrice < 100000,
    );
    expect(found).toBeDefined();
  }, T);

  test("cancelOrder cancels the pending order", async () => {
  ...
  test("getDeals returns recent deals", async () => {
    const to = Date.now();
  ...
  test("getOrders returns recent order history", async () => {
    const to = Date.now();
  ...
  test("raw.market.getAssets returns assets", async () => {
  ...
  test("raw.market.getSymbolCategories returns categories", async () => {
    const cats = await ct.raw.market.getSymbolCategories();
    expect(cats.length).toBeGreaterThan(0);
  }, T);

  test("raw.auth.getApiVersion returns version string", async () => {
  ...
  test("final cleanup: close all EURUSD positions", async () => {
    try {
      await ct.closeSymbol(SYMBOL);
    } catch {
      // No positions
