...
describe("SymbolCache", () => {
  ...
  describe("resolveId", () => {
    ...
    test("throws for unknown symbol", async () => {
      await expect(cache.resolveId("INVALID")).rejects.toThrow(
  ...
  describe("getSymbol", () => {
    ...
    test("throws for unknown symbol ID", async () => {
      await expect(cache.getSymbol(9999)).rejects.toThrow(
        "Symbol id 9999 not found in cache.",
      );
    });
  });
  ...
  describe("edge cases", () => {
    ...
    test("handles empty symbol list", async () => {
      ...
      await expect(c.resolveId("EURUSD")).rejects.toThrow(
