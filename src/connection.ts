...
export class CTraderConnection {
	...
	private readonly eventHandlers = new Map<ConnectionEvent, Set<ConnectionEventHandler>>();
	...
	private handleMessage(msg: DecodedMessage): void {
		...
		if (clientMsgId !== undefined && this.pending.has(clientMsgId)) {
			...
			if (payloadType === PayloadType.ERROR_RES || payloadType === PayloadType.OA_ERROR_RES) {
				req.reject(this.makeError(payload as unknown as OAErrorPayload | undefined));
				return;
			}
		...
		// Unsolicited error
		if (payloadType === PayloadType.ERROR_RES || payloadType === PayloadType.OA_ERROR_RES) {
			this.emit("error", this.makeError(payload as unknown as OAErrorPayload | undefined));
			return;
		}
