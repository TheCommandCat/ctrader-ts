...
describe("codec", () => {
  describe("encodeMessage / decodeMessage roundtrip", () => {
    test("encodes and decodes APP_AUTH_REQ", () => {
      ...
      expect((decoded.payload as Record<string, unknown>).clientId).toBe(
        "test-client",
      );
      expect((decoded.payload as Record<string, unknown>).clientSecret).toBe(
        "test-secret",
      );
    });
    ...
    test("encodes and decodes ACCOUNT_AUTH_REQ", () => {
      ...
      const numericId =
        typeof accountId === "object" && accountId !== null
          ? Number(accountId)
          : accountId;
