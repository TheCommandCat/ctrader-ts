...
export class CTrader {
	...
	async getTrader(): Promise<EnrichedTrader> {
		...
		return {
			...trader,
			leverage: trader.leverageInCents !== undefined ? trader.leverageInCents / 100 : undefined,
		};
	}
	...
	async getState(): Promise<AccountState> {
		...
		const unrealizedPnl = pnlData.pnls.reduce((sum, p) => sum + p.netUnrealizedPnL, 0) / divisor;

		const balance = trader.balance / divisor;
		const equity = balance + unrealizedPnl;

		const usedMargin =
			reconciled.positions.reduce((sum, p) => sum + (p.usedMargin ?? 0), 0) / divisor;
	 ...
	 */
	async getDeals(opts: GetDealsOptions = {}): Promise<{ deals: Deal[]; hasMore: boolean }> {
		const now = Date.now();
		const params: {
	 ...
	 */
	async getOrders(opts: GetOrdersOptions = {}): Promise<{ orders: Order[]; hasMore: boolean }> {
		const now = Date.now();
	...
	async modify(positionId: number, opts: ModifyOptions): Promise<void> {
		...
		if (sltp.relativeStopLoss !== undefined) {
			...
			amendParams.stopLoss =
				pos.tradeData.tradeSide === TradeSide.BUY ? entryPrice - distance : entryPrice + distance;
	...
	async resize(positionId: number, newLots: Lots): Promise<ExecutionEvent | undefined> {
		if (newLots < 0)
			throw new Error("newLots must be >= 0. Use close() to fully close a position.");
		if (newLots === 0)
			throw new Error("newLots must be > 0. Use close() to fully close a position.");
	...
	async watchSpots(
		...
		const idToName = new Map<number, string>(
			ids.map((id, i) => [
				id,
				typeof symbolsOrIds[i] === "string" ? (symbolsOrIds[i] as string) : String(symbolsOrIds[i]),
			]),
		);
	...
	async watchState(
		...
		const offMargin = this.onMarginChanged((event: MarginChangedEvent) => {
			...
			if (pos !== undefined) {
				...
				const usedMargin =
					currentState.positions.reduce((sum, p) => sum + (p.usedMargin ?? 0), 0) /
					10 ** currentState.moneyDigits;
