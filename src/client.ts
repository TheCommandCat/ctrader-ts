...
export class CTrader {
	...
	constructor(config: CTraderClientConfig) {
		const connConfig: CTraderConnectionConfig = { endpoint: config.endpoint };
		if (config.maxReconnectAttempts !== undefined) connConfig.maxReconnectAttempts = config.maxReconnectAttempts;
		if (config.requestTimeoutMs !== undefined) connConfig.requestTimeoutMs = config.requestTimeoutMs;
		if (config.onReconnect !== undefined) connConfig.onReconnect = config.onReconnect;
	...
	get state(): ConnectionState { return this.connection.state; }
	...
	async getState(): Promise<AccountState> {
		...
		const unrealizedPnl = pnlData.pnls.reduce(
			(sum, p) => sum + p.netUnrealizedPnL,
			0,
		) / divisor;
		...
		const usedMargin = reconciled.positions.reduce(
	 ...
	 */
	async getDeals(from?: number, to?: number, maxRows?: number): Promise<{ deals: Deal[]; hasMore: boolean }> {
		const params: { fromTimestamp?: number; toTimestamp?: number; maxRows?: number } = {};
	...
	async getDealsByPosition(positionId: number, from?: number, to?: number): Promise<Deal[]> {
		const params: { fromTimestamp?: number; toTimestamp?: number } = {};
		if (from !== undefined) params.fromTimestamp = from;
		if (to !== undefined) params.toTimestamp = to;
		const { deals } = await this.raw.account.getDealsByPosition(positionId, params);
		return deals;
	}
	...
	/** Order history */
	async getOrders(from?: number, to?: number): Promise<{ orders: Order[]; hasMore: boolean }> {
		const params: { fromTimestamp?: number; toTimestamp?: number } = {};
		if (from !== undefined) params.fromTimestamp = from;
		if (to !== undefined) params.toTimestamp = to;
	 ...
	 */
	async getExpectedMargin(symbol: Symbol, volumesInLots: Lots[]): Promise<{ margins: ExpectedMargin[]; moneyDigits?: number }> {
		const id = await this.symbols.resolveId(symbol);
		return this.raw.account.getExpectedMargin(id, volumesInLots.map(lotsToUnits));
	}

	/** Unrealised P&L for all open positions */
	async getUnrealizedPnl(): Promise<{ pnls: PositionUnrealizedPnL[]; moneyDigits: number }> {
	...
	async getMarginCalls(): Promise<MarginCall[]> { return this.raw.account.getMarginCalls(); }

	/** Update a margin call threshold level */
	async updateMarginCall(marginCall: MarginCall): Promise<void> { return this.raw.account.updateMarginCall(marginCall); }
	 ...
	 */
	async buyLimit(symbol: Symbol, opts: LimitOrderOptions): Promise<ExecutionEvent> {
	 ...
	 * @param opts - Must include limitPrice
	 */
	async sellLimit(symbol: Symbol, opts: LimitOrderOptions): Promise<ExecutionEvent> {
	 ...
	 */
	async buyStop(symbol: Symbol, opts: StopOrderOptions): Promise<ExecutionEvent> {
	 ...
	 * @param opts - Must include stopPrice
	 */
	async sellStop(symbol: Symbol, opts: StopOrderOptions): Promise<ExecutionEvent> {
	 ...
	 */
	async stopLimit(symbol: Symbol, tradeSide: TradeSide, opts: StopLimitOptions): Promise<ExecutionEvent> {
		const id = await this.symbols.resolveId(symbol);
		const volume = lotsToUnits(opts.lots);
		const sltp = await resolveSlTp(
			{ account: this.raw.account, market: this.raw.market, symbols: this.symbols, symbolId: id, volumeInUnits: volume },
			opts.sl,
			opts.tp,
		);
		...
		if (opts.expirationTimestamp !== undefined) p.expirationTimestamp = opts.expirationTimestamp;
	...
	async modify(positionId: number, opts: ModifyOptions): Promise<void> {
		...
		const sltp = await resolveSlTp(
			{ account: this.raw.account, market: this.raw.market, symbols: this.symbols, symbolId, volumeInUnits: volume },
			opts.sl,
			opts.tp,
		);
		...
		if (sltp.relativeStopLoss !== undefined && pos.price !== undefined) {
			...
			amendParams.stopLoss = pos.tradeData.tradeSide === TradeSide.BUY
				? pos.price - distance
				: pos.price + distance;
		}
		...
		if (sltp.relativeTakeProfit !== undefined && pos.price !== undefined) {
			...
			amendParams.takeProfit = pos.tradeData.tradeSide === TradeSide.BUY
				? pos.price + distance
				: pos.price - distance;
		}
	 ...
	 */
	async close(positionId: number, opts?: { lots?: number }): Promise<ExecutionEvent> {
	...
	async closeSymbol(symbol: Symbol): Promise<ExecutionEvent[]> {
		...
		return Promise.all(
			matching.map((p) => this.raw.trading.closePosition(p.positionId, p.tradeData.volume)),
		);
	}
	...
	async closeAll(): Promise<ExecutionEvent[]> {
		const { positions } = await this.raw.account.reconcile();
		return Promise.all(
			positions.map((p) => this.raw.trading.closePosition(p.positionId, p.tradeData.volume)),
		);
	}
	 ...
	 * // later: await stop();
	 */
	async watchSpots(symbolsOrIds: Symbol[], handler: (price: SpotPrice) => void): Promise<() => Promise<void>> {
		const ids = await Promise.all(symbolsOrIds.map((s) => this.symbols.resolveId(s)));
		const idToName = new Map<number, string>(
			ids.map((id, i) => [id, typeof symbolsOrIds[i] === "string" ? (symbolsOrIds[i] as string) : String(symbolsOrIds[i])]),
		);
		...
		const unsub = this.raw.market.onSpot((event: SpotEvent) => {
			...
			const price: SpotPrice = { symbol: idToName.get(event.symbolId) ?? String(event.symbolId), symbolId: event.symbolId };
			if (event.bid !== undefined) { price.bid = event.bid; price.bidDecimal = event.bid / 100000; }
			if (event.ask !== undefined) { price.ask = event.ask; price.askDecimal = event.ask / 100000; }
	 ...
	 */
	async getTrendbars(symbol: Symbol, params: Omit<GetTrendbarsParams, "symbolId">): Promise<{ trendbars: Trendbar[]; hasMore: boolean }> {
		const id = await this.symbols.resolveId(symbol);
	 ...
	 */
	async getTickData(symbol: Symbol, params: Omit<GetTickDataParams, "symbolId">): Promise<{ ticks: TickData[]; hasMore: boolean }> {
		const id = await this.symbols.resolveId(symbol);
	...
	onClientDisconnect(handler: (event: ClientDisconnectEvent) => void): () => void {
		return this.connection.on(PayloadType.CLIENT_DISCONNECT_EVENT, (payload) => {
	...
	onTokenInvalidated(handler: (event: TokenInvalidatedEvent) => void): () => void {
		return this.connection.on(PayloadType.ACCOUNTS_TOKEN_INVALIDATED_EVENT, (payload) => {
	...
	onAccountDisconnect(handler: (event: AccountDisconnectEvent) => void): () => void {
		return this.connection.on(PayloadType.ACCOUNT_DISCONNECT_EVENT, (payload) => {
	...
	async watchDepth(symbolsOrIds: Symbol[], handler: (event: DepthEvent) => void): Promise<() => Promise<void>> {
		const ids = await Promise.all(symbolsOrIds.map((s) => this.symbols.resolveId(s)));
	...
	private async executeMarketOrder(symbol: Symbol, tradeSide: TradeSide, opts: MarketOrderOptions): Promise<Position> {
		const id = await this.symbols.resolveId(symbol);
		const volume = lotsToUnits(opts.lots);
		const sltp = await resolveSlTp(
			{ account: this.raw.account, market: this.raw.market, symbols: this.symbols, symbolId: id, volumeInUnits: volume },
	...
	private async executeLimitOrder(symbol: Symbol, tradeSide: TradeSide, opts: LimitOrderOptions): Promise<ExecutionEvent> {
		const id = await this.symbols.resolveId(symbol);
		const volume = lotsToUnits(opts.lots);
		const sltp = await resolveSlTp(
			{ account: this.raw.account, market: this.raw.market, symbols: this.symbols, symbolId: id, volumeInUnits: volume },
	...
	private async executeStopOrder(symbol: Symbol, tradeSide: TradeSide, opts: StopOrderOptions): Promise<ExecutionEvent> {
		const id = await this.symbols.resolveId(symbol);
		const volume = lotsToUnits(opts.lots);
		const sltp = await resolveSlTp(
			{ account: this.raw.account, market: this.raw.market, symbols: this.symbols, symbolId: id, volumeInUnits: volume },
...
function buildMarketOrderFields(opts: MarketOrderOptions): Record<string, unknown> {
	const f: Record<string, unknown> = {};
	if (opts.trailingStopLoss !== undefined) f["trailingStopLoss"] = opts.trailingStopLoss;
	if (opts.guaranteedStopLoss !== undefined) f["guaranteedStopLoss"] = opts.guaranteedStopLoss;
